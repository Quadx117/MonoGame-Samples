...
class BlackjackCardGame : CardsGame
{
                ...
                for (int playerIndex = 0; playerIndex < players.Count; playerIndex++)
                {
                    if (((BlackjackPlayer)players[playerIndex]).MadeBet)
                    {
                        ...
                        AddDealAnimation(card, animatedHands[playerIndex], true, dealDuration,
                            DateTime.Now + TimeSpan.FromSeconds(dealDuration.TotalSeconds * ((dealIndex * players.Count) + playerIndex)));
                    }
                }
    ...
    private void EndGame()
    {
        ...
        // Add a component for an empty stalling animation. This actually acts as a timer.
        Texture2D texture = Game.Content.Load<Texture2D>(@"Images\youlose");
        animationComponent = new AnimatedGameComponent(this, texture)
        {
            CurrentPosition = new Vector2(Game.GraphicsDevice.Viewport.Bounds.Center.X - (texture.Width / 2),
    ...
    void ResetGame(object obj)
    {
        ...
        {
            if (Game.Components[compontneIndex] != youLooseComponent &&
                Game.Components[compontneIndex] != mainMenuButton &&
