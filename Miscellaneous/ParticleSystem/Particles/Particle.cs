...
internal class Particle
{
    ...
    public Particle(Vector2 position, ParticleData data)
    {
        ...
        _origin = new Vector2(_data.texture.Width / 2,
                              _data.texture.Height / 2);

        if (data.speed != 0)
        {
            _data.angle = MathHelper.ToRadians(_data.angle);
            _direction = new Vector2((float)Math.Sin(_data.angle),
    ...
    {
        _lifespanLeft -= totalSecondsElapsed;
        if (_lifespanLeft <= 0f)
        {
            isFinished = true;
            return;
        ...
        _position += _direction * _data.speed * totalSecondsElapsed;
    }
